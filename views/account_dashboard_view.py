...
class AccountDashboardView(View):
    ...
    def __init__(self, master, view_model=None):
        ...
        self.title = tk.Label(self.side_bar, font=("Arial", 24, "bold"))

        self.navs = [
            ("Inicio", lambda: tk.Frame(self.body)),
        ...
        self.nav_buttons = [
            tk.Button(self.side_bar, text=label) for label, builder in self.navs
        ]
        self.nav_buttons.append(tk.Button(self.side_bar, text="Regresar <"))
    ...
    def on_update(self, changed):
        ...
        if changed("color_schema"):
            ...
            self.side_bar.config(bg=color_schema.SURFACE_CONTAINER)
            self.title.config(
